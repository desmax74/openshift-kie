...
public class PodConsumer<T> implements EventConsumer  {
    ...
    public PodConsumer(String id, Properties properties, ConsumerHandle consumerHandle) {
        this.id = id;
    ...
    public void subscribe(String groupId,
        ...
        //consumer.subscribe(Collections.singletonList(topic), new PartitionListener(consumer, offsets));// for production
        ...
        try {
            ...
            try {
                ...
                for (Map.Entry<TopicPartition, OffsetAndMetadata> entry : offsets.entrySet())
                    logger.info("Consumer %s - partition %s - lastOffset %s\n", this.id, entry.getKey().partition(), entry.getValue().offset());
                //Store offsets
                OffsetManager.store(offsets);//@TODO store on infinispan
            } finally {
                consumer.close();
            }
        }
    ...
    public boolean assign(String topic,
        ...
        consumer = new KafkaConsumer<>(ConsumerConfig.getConfig("", properties.getProperty("desererializerClass"), autoCommit));
    ...
    private void consume(int size, boolean commitSync) {
        ...
        for (ConsumerRecord<String, T> record : records) {
            ...
            //@TODO checks how to handle this with multiple pods, store next offset to commit
